...
, ofcp = module.exports = {
    ...
    evalFrontHand: function(hand) {
        ...
        var values = hand.map(function(c) {
            return _.isNumber(c) ? c : PokerEvaluator.CARDS[c.toLowerCase()]
        })
    ...
    isFoul: function(back, mid, front) {
        ...
        , sortNumericAsc = function(a, b) { return a - b }
    ...
    settle: function(hand1, hand2) {
        ...
        var back = this.settleBack(hand1.back, hand2.back)
        , mid = this.settleMid(hand1.mid, hand2.mid)
        , front = this.settleFront(hand1.front, hand1.front)
        , res = back + mid + front

        // player one scoops
        if (back > 0 && mid > 0 && front > 0) {
            res += 3
        }
